...
public class ItemUseProgressBars {
	...
	public static class ItemUseProgressBar {
        ...
        //~ if <26.1 'graphics.blitSprite(RenderPipelines.GUI_TEXTURED, ' -> 'graphics.blitSprite(' {
		public void extractBackground(@NotNull final GuiGraphicsExtractor graphics) {
	...
	private enum ItemUseProgressBarType {
		BOW(true, stack -> ReArm.CONFIG.bow.chargeBar.get() && ReArmItems.isBow(stack), p -> ReArm.CONFIG.bow.enablePerfectShot.get() ? 20 + ReArm.CONFIG.bow.perfectShotTimeframe.get() : 20),
		...
		AXE(false, stack -> ReArm.CONFIG.axe.chargeBar.get() && stack.is(ItemTags.AXES), p -> 10),
		TRIDENT(false, stack -> ReArm.CONFIG.trident.chargeBar.get() && stack.is(Items.TRIDENT), p -> 10);
