...
public class DataPatches {
	...
	public static void init() {
							...
							damage.forEach(element -> {
								...
								if (object.has("requirements")) {
									...
									if (reqElem.isJsonObject()) {
										...
										predicate.addProperty(/*? <26.2 {*//*"type"*//*?} else {*/"minecraft:entity_type"/*?}*/, "#minecraft:sensitive_to_impaling");
										...
										if (requirements.equals(match)) {
											...
											terms.get(0).getAsJsonObject().getAsJsonObject("predicate").addProperty(/*? <26.2 {*//*"type"*//*?} else {*/"minecraft:entity_type"/*?}*/, "#minecraft:sensitive_to_impaling");
											//? <26.2 {
		...
		// Recipes
		if (ReArm.CONFIG.tweaks.craftTippedArrowsWithRegularPotions.get()) MixsonHelper.registerSingleJson(
        ...
        /*MixsonHelper.registerMultiJson(
                ...
                index -> index.id().toString().startsWith("rearm:recipe/"),
                context -> {
                    String path = context.getIndex().id().getPath();
                    if (path.equals("recipe/netherite_shield_decoration")) context.markForDeletion(true);
                    else if (path.endsWith("_smithing")) {
                        JsonObject obj = context.getFile().getAsJsonObject();
                        JsonObject additionObj = new JsonObject();
                        JsonObject baseObj = new JsonObject();
                        JsonObject templateObj = new JsonObject();
                        additionObj.addProperty("item", obj.get("addition").getAsString());
                        baseObj.addProperty("item", obj.get("base").getAsString());
                        templateObj.addProperty("item", obj.get("template").getAsString());
                        obj.add("addition", additionObj);
                        obj.add("base", baseObj);
                        obj.add("template", templateObj);
                    }
                }
        );
        *///?}
