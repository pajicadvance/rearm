...
public class NeoforgeEntrypoint {
	...
	private static void initCommonResources(AddPackFindersEvent event) {
		event.addPackFinders(
				ReArm.id("resourcepacks/" + ReArm.PACK_VERSION + "/dp"),
				PackType.SERVER_DATA,
				Component.literal("ReArm " + ReArm.PACK_VERSION + " Data Pack"),
				PackSource.BUILT_IN,
				true,
				Pack.Position.TOP
		);
	...
	private static void initNetworkEvents(RegisterPayloadHandlersEvent event) {
		...
		registrar.playToServer(
				...
				(payload, context) ->
						BashAbility.handleBash(payload, (ServerPlayer) context.player())
		);
